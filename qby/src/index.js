...
import homeEdit from './models/contentCenter/homeEdit.js';
...
const models = [
  goodsLogList,productGoodsList,
  productEditGoods,countryManage,
  internalSort,cTipAddGoods,
  cTipGoodsList,bTipAddGoods,
  bTipGoodsList,addGoods,
  baseGoodsDetail,baseGoodsList,
  cExplain,bondManage,
  userorders,cUserManage,
  allth,toAudit,
  account,downlaod,
  dataposorder,datasporder,
  datas,dataclassdes,
  datagodes,dataspfen,
  datasphiscun,dataspcun,
  dataspsell,mdDivide,
  thList,bStock,
  onlineStock,specs,
  brand,fenlei,
  ...
  ordermd,IndexPage,
  tab,users,
  operatemember,operateinout,
  operatesupplier,operatebanner,
  h5config,dataposManage,
  onlinegood,marketResource,
  cBanner,userFeedBack,
  serverBill,coupon,
  bPush,cPush,
  cTimer,appBase,
  supplyinout,withdraw,
  bAnswer,bAddAnswer,
  onAudit,sellManage,
  freightDetail,cServerOrder,
  shareRate,shareTotal,
  shareMail,orderuser,
  exchangeAct,
  //运营中心
  bActPrice,bDown,
  cDown,themeAct,
  pageConfig,market,
  homeEdit,commodityFlow,bannerSet
]
models.forEach(m => {
  return app.model(m)
})
