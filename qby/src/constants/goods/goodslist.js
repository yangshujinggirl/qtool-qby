...
class Goodlist extends React.Component {
    ...
    onShowSizeChange=(current,size)=>{
        ...
        value.limit=Number(size)-1
        value.currentPage=current      
        this.props.dispatch({
            type:'goods/fetch',
            payload:{code:'qerp.web.pd.spu.query',values:value}
        })
    }
    render() {
        return (
            <div>
               <ul className='listbox clearfix'>
                    ...
                    <div>
                        <Pagination showSizeChanger onShowSizeChange={this.onShowSizeChange.bind(this)} total={this.props.total} pageSize={this.props.limit} pageSizeOptions={['16','50','100','200']}/>
                    </div>
               </div>
        );
    }
