...
const sellingPoints = (
  <Tooltip
    placement="top"
    title="选填项，如填写则前端将会展示卖点，而不是商品名称"
  >
    商品卖点&nbsp;
...
const pdSpuInv = (
  <Tooltip
    placement="top"
    title="即为仓库库存。若库存不为0，则所有用户都可以买这个商品。"
  >
    B端在售库存&nbsp;
    <Icon type="exclamation-circle-o" />
  </Tooltip>
);
const outOfStockShopNum = (
  <Tooltip
    placement="top"
    title="即为该门店没有此商品，若B端在售库存为0，则选择此门店的用户会看到补货中"
  >
    缺货门店&nbsp;
    <Icon type="exclamation-circle-o" />
  </Tooltip>
...
const tags = (
  <Tooltip placement="top" title={tagsTit}>
    商品标签&nbsp;
    <Icon type="exclamation-circle-o" />
  </Tooltip>
);
export function columnsFun(form, handleBlur) {
  return [
      ...
      render: (text, record, index) => {
        ...
        return (
          <div className="img-wrap">
            {record.pdSpuPic && <img src={`${fileDomain}${record.pdSpuPic}`} />}
          </div>
        );
      }
    },
    ...
    },
    {
      ...
      render: (text, record, index) => {
        return (
          <span onClick={() => record.onOperateClick("delete")} className="cr">
            删除
          </span>
        );
      }
