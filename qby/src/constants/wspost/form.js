...
class App extends React.Component {
	constructor(props) {
		super(props);
		this.state = { orderNo: '', wsExpressId:'请选择',expressMailno:'', shippingFee:'' ,wsExpressIdes:'1',shopName:null,type:null,shopId:null} ;
	}
	...
	orderNoHindonKeyUp=(e)=>{
		if(e.keyCode==13){
			//数据请求
			var values={orderNo:e.target.value}
			const result=GetServerData('qerp.web.ws.order.orderno.query',values)
			result.then((res) => {
				return res;
			}).then((json) => {
				if(json.code=='0'){
